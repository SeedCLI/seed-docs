...
const clientReferences = {
  "f5d2887272df": async () => {
    const m = await import("./assets/worker-entry-CnPbAXF_.js").then((n) => n.w);
...
async function handleSsr(rscStream, navContext, fontData) {
  return runWithNavigationContext(async () => {
    ...
    try {
      ...
      const { createElement, Fragment } = await Promise.resolve().then(() => React);
      let insertedHTML = "";
      for (const el of insertedElements) {
        try {
          insertedHTML += server_edgeExports.renderToStaticMarkup(createElement(Fragment, null, el));
        } catch {
        }
      }
      let fontHTML = "";
