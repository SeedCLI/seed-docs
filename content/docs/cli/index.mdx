...
Runs your entry point with `node --watch` for instant feedback. For TypeScript entries (`.ts`, `.tsx`, `.mts`, `.cts`), `seed dev` also loads the [`tsx`](https://tsx.is) ESM loader (`node --watch --import tsx ...`) so you can use the standard TypeScript ESM conventions:
